...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8044
EXPOSE 8044
...
| `EXPOSE 8044` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8044"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8044"| APIFACTURAS
    APIFACTURAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15444"| POSTGRES
```
...
```yaml
  postgres:
    ...
    ports:
      - "15444:5432"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8044`, `localhost:15444`); entre contenedores, nombres de
...
docker network create proyecto_construccion_de_software3_default

# 2. postgres
docker run -d --name postgres --network proyecto_construccion_de_software3_default --restart unless-stopped `
  ...
  -p 15444:5432 postgres:16-alpine
...
docker run -d --name api-facturas --network proyecto_construccion_de_software3_default --restart unless-stopped `
...
```yaml
URL_API: http://api-facturas:8044
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8044` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8044` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8044:8044`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8044`, el front
...
```powershell
docker network inspect proyecto_construccion_de_software3_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_construccion_de_software3` produce
la red `proyecto_construccion_de_software3_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_construccion_de_software3-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_construccion_de_software3_default has active endpoints (name:"proyecto_construccion_de_software3-api-facturas-1" …)
```
